<template>
	<UForm
		...
		@submit="handleSubmit"
	>
		<UFormGroup name="packageJsonUrl" size="lg">
			<template #label> Link to your package.json </template>
			<template #description>
				We will fetch the dependencies from your package.json file.
			</template>
			<UInput
		...
		<UFormGroup hint="Optional" name="lockfileUrl" size="lg">
	...
	<UCard v-if="isLoadingPackageJson" class="flex items-center justify-center">
		Loading...
	</UCard>
	<UCard v-if="isErrorPackageJson" class="flex items-center justify-center">
		{{ packageJsonError }}
	</UCard>
</template>
